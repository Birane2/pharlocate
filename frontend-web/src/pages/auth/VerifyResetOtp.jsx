...
function VerifyResetOtp() {
  ...
  const handleSubmit = async (event) => {
    ...
    setLoading(true);
    try {
      const response = await verifyPasswordResetOtp(normalizedEmail, otp);
      ...
      window.setTimeout(() => navigate("/reset-password/new"), 650);
    } catch (err) {
      setError(
        getApiErrorMessage(
          err,
      ...
      <section className="relative w-full max-w-[450px] rounded-2xl border border-[#E2E8F2] bg-white/95 p-5 shadow-[0_20px_60px_rgba(28,43,74,0.10)] backdrop-blur sm:p-6">
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          ...
          <div>
            ...
            <div className="flex h-12 items-center gap-3 rounded-xl border border-[#E2E8F2] bg-white px-3.5 shadow-sm transition focus-within:border-[#2F6E9E] focus-within:ring-4 focus-within:ring-[#2F6E9E]/10">
              <FontAwesomeIcon icon={faKey} className="h-4 w-4 text-[#2F6E9E]" />
              <input
                type="text"
                inputMode="numeric"
                maxLength={6}
                ...
                placeholder="123456"
                required
                className="h-full min-w-0 flex-1 bg-transparent text-sm font-medium tracking-[0.35em] text-[#1C2B4A] outline-none placeholder:tracking-normal placeholder:text-[#9CA3AF]"
              />
            </div>
          </div>
