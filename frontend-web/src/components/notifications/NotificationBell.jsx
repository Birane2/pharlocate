...
function NotifToast({ count, onClose }) {
  useEffect(() => {
  ...
  return (
    <div
      className="fixed bottom-5 right-5 z-[9999] flex items-center gap-3 rounded-2xl border border-[#2F6E9E]/20 bg-white px-4 py-3 shadow-xl animate-[slideUp_0.3s_ease]"
      style={{ minWidth: 240 }}
    >
      <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#2F6E9E]/10 text-[#2F6E9E]">
        <FontAwesomeIcon icon={faBell} className="h-4 w-4" />
      </span>
      <div className="flex-1">
        <p className="text-sm font-bold text-[#1C2B4A]">
          {count === 1 ? "1 nouvelle notification" : `${count} nouvelles notifications`}
        </p>
        <p className="text-xs text-[#6B7280]">Cliquez sur la cloche pour voir</p>
      </div>
      <button
        type="button"
        onClick={onClose}
        className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg text-[#9CA3AF] transition hover:text-[#374151]"
        aria-label="Fermer"
      >
...
function NotificationBell({ notificationsPath = "/pharmacien/notifications" }) {
  ...
  const fetchCount = useCallback(async () => {
    try {
      ...
      // Show toast only after first fetch (prevCountRef initialised) and only
      // when the bell is closed so we don't interrupt the open panel.
      if (prevCountRef.current !== null && !open) {
  ...
  useEffect(() => {
    if (open) {
      fetchDropdown();
      setToast(null); // dismiss toast when panel opens
      // Reset prev count so next poll baseline is current server value
  ...
  return (
    <>
      ...
      {/* Toast portal */}
      {toast && (
        <NotifToast count={toast.count} onClose={() => setToast(null)} />
      )}
